...
pub fn show(header: &MavHeader, msg: &MavMessage) {
    match msg {
        ...
        _ => generic(header, msg),
    }
}

fn heartbeat(header: &MavHeader, data: &HEARTBEAT_DATA) {
    println!(
        "HEARTBEAT [{}/{}] type={:?}, autopilot={:?}, status={:?}",
...
fn generic(header: &MavHeader, msg: &MavMessage) {
    let formatted = format!("{:?}", msg);
    let name = formatted.split('(').next().unwrap_or("UNKNOWN");
    println!("{} [{}/{}]", name, header.system_id, header.component_id);
}
